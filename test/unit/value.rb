...
Expectations do
  ...
  expect NoMethodError do
    Class.new(Value){ |c| c.values :a }.new(1).a
  end

  expect 1 do
    Class.new(Value){ |c| c.values :a }.new(1).instance_eval{ a }
  end

  expect 1 do
    Class.new(Value){ |c| c.values :a }.new(1).instance_variable_get(:@a)
  end

  expect true do
    c = Class.new(Value){ values :a, :b, :c }
  ...
  expect(/\A#<Class:0x[[:xdigit:]]+>\.new\(1, 2, 3\)\z/) do
    Class.new(Value){ values :a, :b, :c }.new(1, 2, 3).inspect
  end
end
